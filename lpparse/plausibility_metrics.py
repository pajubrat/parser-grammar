...
class PlausibilityMetrics:
    ...
    def filter_and_rank(self, ps, w):
        """
        Decides which nodes are prioritized when [w] is merged the existing representation [ps].
        Contains and implements cognitive parsing principles.

        First, the function filters out nodes that are not active in the working memory.
        Then the remaining nodes are filtered on the basis of various filtering principles.
        Then the remaining nodes are ranked by using cognitive parsing principles.
        Finally, a complete list of  nodes is composed which is the sum of ranked sites plus
        the inactive nodes (that are not processed). THis list is returned to the parser, which will
        then pick up the first solution. If the parser backtracks, it will explore the remaining solutions
        in the order generated by this module.
        """
        ...
        if self.word_internal(ps, w):
            ...
            # Partition the nodes into active and inactive
