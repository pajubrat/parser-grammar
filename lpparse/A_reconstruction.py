...
class A_reconstruction:
    ...
    def reconstruct(self, spec):
        if spec == spec.container_head().licensed_phrasal_specifier() or self.VP_fronting_in_Finnish(spec):
    ...
    def VP_fronting_in_Finnish(self, spec):
        return spec == next((spec for spec in spec.container_head().edge() if {'A/inf', 'VA/inf'} & spec.head().features), None)
