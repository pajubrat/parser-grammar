...
class LexicalInterface:
    ...
    def apply_redundancy_rules(self, current_features):
        ...
        def new_features(current_features):
            ...
            """
            new = set()
            for rule in [rule for rule in self.redundancy_rules.keys() if rule_trigger(rule, current_features)]:
                new = new | {feature for feature in self.redundancy_rules[rule] if default_rule(feature, current_features)}
            return new
        ...
        while new_features(current_features):
            current_features = current_features | new_features(current_features)

        return self.combine_features(current_features)
