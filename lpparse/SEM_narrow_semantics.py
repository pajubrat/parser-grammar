...
class NarrowSemantics:
    def __init__(self, speaker_model):
        ...
        self.semantic_action = \
            {'GLOBAL': {'Remove': self.global_cognition.remove_object,
                        ...
                        'Presentation': self.global_cognition.presentation},
    ...
    def postsyntactic_semantic_interpretation(self, X):
        ...
        if self.speaker_model.settings.retrieve('general_parameter_calculate_assignments', True):
            n, weighted_assignments, summary_string = self.quantifiers_numerals_denotations.reconstruct_assignments(X)
            ...
            self.speaker_model.results.store_output_field('Binding', summary_string)
    ...
    def inventory_projection_(self, X, space):
        ...
        # Projects objects into semantic spaces and then into the global discourse inventory

        if not X.copied and space != 'GLOBAL':  # Global objects are projects inside this function in connection with NS projections
            ...
            if X.complex():
                ...
                if self.semantic_action[space]['Accept'](X):
                    ...
                    # Create a new semantic objects based on the semantic space and its Project function
                    # The attributes are generated by the semantic spaces

                    new_semantic_object_dict = self.semantic_action[space]['Project'](X, idx)

                    # Projects a global object

                    self.semantic_action['GLOBAL']['Project'](X, new_semantic_object_dict.copy())

                    # For heuristic purposes so that referential arguments are recognized by binding theory
