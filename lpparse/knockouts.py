...
def knockout_working_memory(funct):
    def knockout(self, ps):
        ...
        if 'working_memory' in self.controlling_parser_process.local_file_system.settings:
            if self.controlling_parser_process.local_file_system.settings['working_memory']:
