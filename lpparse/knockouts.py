...
def knockout_working_memory(funct):
    def knockout(self, ps):
        ...
        if 'working_memory' in self.speaker_model.local_file_system.settings:
            if self.speaker_model.local_file_system.settings['working_memory']:
...
def knockout_filter(funct):
    def knockout(self, list_of_sites, w):
        if 'filter' in self.speaker_model.local_file_system.settings:
            if not self.speaker_model.local_file_system.settings['filter']:
...
def knockout_lexical_ranking(funct):
    def knockout(self, site):
        if 'lexical_anticipation' in self.speaker_model.local_file_system.settings:
            if not self.speaker_model.local_file_system.settings['lexical_anticipation']:
...
def knockout_extra_ranking(funct):
    def knockout(self, site):
        if 'extra_ranking' in self.speaker_model.local_file_system.settings:
            if not self.speaker_model.local_file_system.settings['extra_ranking']:
...
def knockout_baseline_weighting(funct):
    def knockout(self, weighted_list):
        if 'closure' in self.speaker_model.local_file_system.settings:
            return funct(self, weighted_list, method=self.speaker_model.local_file_system.settings['closure'])
        else:
