...
class Discourse:
    ...
    def create_topic_gradient(self, main_arguments):
        ...
        for idx in self.narrow_semantics.semantic_bookkeeping:
            if idx in main_arguments and not self.narrow_semantics.semantic_bookkeeping[idx]['Operator']:
                topic_lst.append((self.narrow_semantics.semantic_bookkeeping[idx]['Order gradient'], idx))
        ...
        for topic, idx in topic_lst:
            if 'Marked gradient' in self.narrow_semantics.semantic_bookkeeping[idx]:
                if self.narrow_semantics.semantic_bookkeeping[idx]['Marked gradient'] == 'High':
                    marked_topic_lst.append(idx)
                    topic_lst_.remove((self.narrow_semantics.semantic_bookkeeping[idx]['Order gradient'], idx))
                if self.narrow_semantics.semantic_bookkeeping[idx]['Marked gradient'] == 'Low':
