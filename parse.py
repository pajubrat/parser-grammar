...
def parse_test_set(test_set_name='example/example_test_set.txt',
    ...
    print(f'Loading test sentences from file "{test_set_name}".')
    ...
    for language in lang_guesser.languages:
        ...
        parsers[language] = PCBParser(context)
    ...
    print(datetime.datetime.now())
    print(f'Test sentences were loaded from file "{test_set_name}".')
    print(f'Logs were written to file "{log_file_name}".')
    print(f'Lexicon was read from file "{test_set_name[:-4]}_lexicon.lex".')

    # Stamp the output file
    results_file = open(results_file_name, "w")
    results_file.write(f'Parser-Grammar v. {version}\n')
